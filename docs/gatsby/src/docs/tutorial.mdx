...
|`addressFilter`|`"RECEIVER_ONLY"`|filters returned transactions to sender or reciever if specified. Optional, defaults to `"SENDER_OR_RECEIVER"`|
...
First, save the reponse from the previous API call in a file:
...
Then, make a second API call to get the rest of the transactions:
...
Both of these API calls pipe the response to a utility called `jq` to nicely format it before we save it to a file.
We'll cover `jq` further in the next section.
